module Card = struct
  ...
  let allSpades =
    let rec aux value acc = match value with
      | Value.As -> acc@[newCard Value.As Color.Spade]
      | v -> aux (Value.next v) (acc@[newCard v Color.Spade])
    in aux Value.T2 []

  let allHearts =
    let rec aux value acc = match value with
      | Value.As -> acc@[newCard Value.As Color.Heart]
      | v -> aux (Value.next v) (acc@[newCard v Color.Heart])
    in aux Value.T2 []

  let allDiamonds =
    let rec aux value acc = match value with
      | Value.As -> acc@[newCard Value.As Color.Diamond]
      | v -> aux (Value.next v) (acc@[newCard v Color.Diamond])
    in aux Value.T2 []

  let allClubs =
    let rec aux value acc = match value with
      | Value.As -> acc@[newCard Value.As Color.Club]
      | v -> aux (Value.next v) (acc@[newCard v Color.Club])
    in aux Value.T2 []
