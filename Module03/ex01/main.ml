let print_next card_value =
  ...
  print_endline ("Value.next for card value " ^ (Value.toStringVerbose card_value) ^ " is : " ^ n)

let print_previous card_value =
  let n = Value.toStringVerbose (Value.previous card_value) in
  print_endline ("Value.previous for card value " ^ (Value.toStringVerbose card_value) ^ " is : " ^ n)
