...
let decode_arn r =
  let rec aux triplets = match triplets with
    | (U,A,A)::(U,A,G)::(U,G,A)::tail -> Stop::[]
    | (G,C,A)::(G,C,C)::(G,C,G)::(G,C,U)::tail -> Ala::(aux tail)
    | (A,G,A)::(A,G,G)::(C,G,A)::(C,G,C)::(C,G,G)::(C,G,U)::tail -> Arg::(aux tail)
    | (A,A,C)::(A,A,U)::tail -> Asn::(aux tail)
    | (G,A,C)::(G,A,U)::tail -> Asp::(aux tail)
    | (U,G,C)::(U,G,U)::tail -> Cys::(aux tail)
    | (C,A,A)::(C,A,G)::tail -> Gln::(aux tail)
    | (G,A,A)::(G,A,G)::tail -> Glu::(aux tail)
    | (G,G,A)::(G,G,C)::(G,G,G)::(G,G,U)::tail -> Gly::(aux tail)
    | (C,A,C)::(C,A,U)::tail -> His::(aux tail)
    | (A,U,A)::(A,U,C)::(A,U,U)::tail -> Ile::(aux tail)
    | (C,U,A)::(C,U,C)::(C,U,G)::(C,U,U)::(U,U,A)::(U,U,G)::tail -> Leu::(aux tail)
    | (A,A,A)::(A,A,G)::tail -> Lys::(aux tail)
    | (A,U,G)::tail -> Met::(aux tail)
    | (U,U,C)::(U,U,U)::tail -> Phe::(aux tail)
    | (C,C,C)::(C,C,A)::(C,C,G)::(C,C,U)::tail -> Pro::(aux tail)
    | (U,C,A)::(U,C,C)::(U,C,G)::(U,C,U)::(A,G,U)::(A,G,C)::tail -> Ser::(aux tail)
    | (A,C,A)::(A,C,C)::(A,C,G)::(A,C,U)::tail -> Thr::(aux tail)
    | (U,G,G)::tail -> Trp::(aux tail)
    | (U,A,C)::(U,A,U)::tail -> Tyr::(aux tail)
    | (G,U,A)::(G,U,C)::(G,U,G)::(G,U,U)::tail -> Val::(aux tail)
    | _::tail -> aux tail
    | [] -> []
  in
...
let () =
  print_endline (string_of_protein (decode_arn ([G;C;A;G;C;C;G;C;G;G;C;U])));
  print_endline (string_of_protein (decode_arn (generate_rna (generate_helix 1000))));
  print_endline (string_of_protein (decode_arn  [U;A;A;U;A;G;U;G;A;A;U;G]))